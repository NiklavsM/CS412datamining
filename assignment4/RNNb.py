...
W0 = theano.shared(numpy.asarray(rng.normal(loc=0.0, scale=0.1, size=(word_embedding_size, recurrent_size))), 'W0')
W1 = theano.shared(numpy.asarray(rng.normal(loc=0.0, scale=0.1, size=(recurrent_size, recurrent_size))), 'W1')
...
def rnn_step(x, h_prev, W0, W1):
    return theano.tensor.tanh(theano.tensor.dot(h_prev, W0) + theano.tensor.dot(x, W1))
...
updates = [
    ...
    (W0, W0 - .01 * theano.tensor.grad(cost, W0)),
    (W1, W1 - .01 * theano.tensor.grad(cost, W1))
]
