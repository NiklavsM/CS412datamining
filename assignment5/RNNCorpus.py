...
def rnn_step(x, h_prev, W_xh, W_hh):
    return theano.tensor.tanh(theano.tensor.dot(h_prev, W_xh) + theano.tensor.dot(x, W_hh))
...
data_train = [(word2id[label], sentence2ids(sentence.split(), word2id)) for label, sentence in
              sentences_train]  # here we need to convert
data_test = [(word2id[label], sentence2ids(sentence.split(), word2id)) for label, sentence in
